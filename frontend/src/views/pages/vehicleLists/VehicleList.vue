  ...
  <div>
    <CCol v-if="!!vehicleList">
      <CRow class="mb-3 ml-0">
        <h1 class="mb-0 d-flex align-items-center" v-if="!editingTitle">
          {{ vehicleList.title }}
        </h1>
        <CButton
          variant="outline"
      ...
      <CRow class="ml-0 mr-0 d-flex justify-content-between">
        <CRow class="m-0">
          ...
          <router-link to="/inventory">
            <CButton color="primary" variant="outline" class="m-3" size="sm"
              >Add vehicles to list</CButton
            >
          </router-link>
        </CRow>
        <CRow class="m-0">
    ...
    rowClicked(vehicle, index, column, e) {
      if (!this.removingVehicles) {
        let routeData = this.$router.resolve({
          ...
          query: { vehicleSelected: vehicle._id },
        });
